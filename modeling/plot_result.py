...
# Keys used to align rows when averaging across seeds
METRIC_KEYS = {
    1: ['backpack_size'],
    2: ['patchUniformity'],
    3: ['distance_bin'],
    4: ['energy_bin'],
    5: ['turn', 'partner_helped_last'],
}

METRIC_NAMES = {
    1: 'metric1_backpack',
    2: 'metric2_patchuniformity',
    3: 'metric3_distance',
    4: 'metric4_energy',
    5: 'metric5_reciprocity',
}
...
def load_metric_csv_mean(metric_num, reward_mode, tags):
    """Load and average metric CSVs across multiple tags, aligned by bin keys."""
    name = METRIC_NAMES[metric_num]
    keys = METRIC_KEYS[metric_num]
    dfs = []
    for tag in tags:
        path = f"results/metrics_{reward_mode}_{tag}_{name}.csv"
    ...
    if len(dfs) == 1:
        return dfs[0].copy()
    # Align by keys and average agent_helping_rate
    base = dfs[0][keys + ['human_helping_rate']].copy().set_index(keys)
    agent_cols = [df.set_index(keys)['agent_helping_rate'] for df in dfs]
    agent_stack = pd.concat(agent_cols, axis=1)
    base['agent_helping_rate'] = agent_stack.mean(axis=1)
    base['agent_helping_std']  = agent_stack.std(axis=1)
    return base.reset_index()


def get_data(metric_num, reward_mode, tag, tags):
    """Return averaged df if tags provided, else single-tag df."""
    if tags:
        return load_metric_csv_mean(metric_num, reward_mode, tags)
    return load_metric_csv(metric_num, reward_mode, tag)


def plot_metric_1(tag="", tags=None):
    fig, axes = plt.subplots(1, 5, figsize=(20, 5))
    fig.suptitle('Helping rate by backpack size: Agent vs. Human', fontsize=14, fontweight='bold')
    all_rates, data_map = [], {}
    for m in REWARD_MODES:
        df = get_data(1, m, tag, tags)
        if df is not None:
            data_map[m] = df
    ...
    plt.tight_layout()
    suffix = f"_{tag}" if tag else (f"_{'_'.join(tags)}" if tags else "")
...
def plot_overall_helping_rate(tag="", tags=None):
    ...
    if tags:
        ...
        for m in REWARD_MODES:
            rates = []
            for t in tags:
                p = f"results/overall_helping_rate_{t}.csv"
                if os.path.exists(p):
                    odf = pd.read_csv(p).drop_duplicates("reward_mode", keep="last")
                    row = odf[odf["reward_mode"] == m]
                    if not row.empty:
                        rates.append(row["agent_rate"].iloc[0])
            if rates:
                agent_rates[m] = np.mean(rates)
                agent_stds[m] = np.std(rates)
    else:
...
def plot_fit_scores(tag="", tags=None):
...
def main():
    ...
    parser.add_argument("--tag", type=str, default="", help="Single results tag, e.g. 'v4'")
    parser.add_argument("--tags", type=str, nargs="+", default=None,
                        help="Multiple tags to average across seeds, e.g. --tags v4 v4.1 v5")
    args = parser.parse_args()
    tag = args.tag
    tags = args.tags
    ensure_figures_dir()
    label = f"tags={tags}" if tags else f"tag='{tag}'"
    print(f"Generating figures ({label})...")
    plot_metric_1(tag=tag, tags=tags)
    plot_metric_2(tag=tag, tags=tags)
    plot_metric_3(tag=tag, tags=tags)
    plot_metric_4(tag=tag, tags=tags)
    plot_metric_5(tag=tag, tags=tags)
    plot_overall_helping_rate(tag=tag, tags=tags)
    plot_fit_scores(tag=tag, tags=tags)
    print("Done!")
